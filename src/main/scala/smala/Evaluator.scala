...
trait Evaluator[T <: Tree, A] {
  ...
  implicit def nodeOrder(implicit ev: Order[A]): Order[tree.Node] =
    new Order[tree.Node] {
      def compare(x: tree.Node, y: tree.Node) = ev.compare(value(x), value(y))
    }

  implicit def nodePartialOrder(implicit ev: PartialOrder[A]): PartialOrder[tree.Node] = 
    new PartialOrder[tree.Node] {
      def partialCompare(x: tree.Node, y: tree.Node) = ev.partialCompare(value(x), value(y))
    }
}
